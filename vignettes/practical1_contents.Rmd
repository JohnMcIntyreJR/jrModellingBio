...
The following code will load a data set into your current session which contains a data frame of of measurements of sequence recognition from yeast proteins with different subcellular localisations[^1].
...
a) Carry out a $\chi^2$ goodness of fit test on the zodiac data. Are yeast proteins distributed uniformly across subcellular localisations?
...
a) Perform the $\r^2$ correlation test to determine if the two measurements of signal sequences `mcg` and `gvh` are correlated.
...
c) Now plot the data as a scatter plot using `ggplot2` and annotate the plot with the `annotate()` function adding the correlation coefficient onto the figure. __Hint: you can create a nice label with the `paste()` function to stick bits of text together including variables you have created.__
