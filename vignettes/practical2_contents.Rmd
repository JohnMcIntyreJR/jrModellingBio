...
\newthought{Lets now} a more sensible model with some different data. We will look at the occurrence of infectious disease illnesses across three states in the USA caused by different microbial pathogens. Load the `outbreaks` dataset:
...
2. Try fitting the model again but with log transformed illnessess. __Hint:__ use `log(illnesses)` in place of `illnesses`. Now re-exmamine the qq-plot? Have we fixed the problem?
...
These next few sections are quite tricky, contain concepts that we didn't cover and are aimed at a very specific use case. If you don't see yourself making figure like Figure XXX then feel free to skip these and move on to question 6 on principal components analysis.
...
4. Now convert your `hclust` object into a `tbl_graph` object using the `as_tbl_graph()` function from `tidygraph`.
...
6. Finally make the graph figure in 
...
- (a) reduce the complexity of the amino acid sequence properties 
